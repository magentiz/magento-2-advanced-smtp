...
class UpgradeSchema implements \Magento\Framework\Setup\UpgradeSchemaInterface
{
    ...
    public function upgrade(SchemaSetupInterface $setup, ModuleContextInterface $context)
    {
        ...
        if ($installer->tableExists('mageplaza_smtp_log')) {
            if (! $connection->tableColumnExists('mageplaza_smtp_log', 'is_save_attachment')) {
                $connection->addColumn($setup->getTable('mageplaza_smtp_log'), 'is_save_attachment', [
                    'type'     => Table::TYPE_SMALLINT,
                    'nullable' => false,
                    'default'  => 0,
                    'length'   => null,
                    'comment'  => 'Is Save Attachment'
                ]);
            }

            if (! $connection->tableColumnExists('mageplaza_smtp_log', 'attachment')) {
                $connection->addColumn($setup->getTable('mageplaza_smtp_log'), 'attachment', [
